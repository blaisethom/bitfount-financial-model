...
export const salesModel = defineModel({
  ...
  sources: {
    ...
    hubspot_pipeline_t: apiSource({
      adapter: 'hubspot_pipeline',
      schema: S.hubspot_pipeline,
      description:
        'HubSpot pipeline value per (TA × line item × month). Populated via the ' +
        '"Sync from HubSpot" button — assigns each deal to a TA via ' +
        '`HubSpotSyncAssumptions.taAllocation` (regex rules on deal name + per-deal ' +
        'manual overrides). Stored in `ModelInput.hubspot` between sessions; ' +
        'flattenModelInput() expands it into rows at evaluation time.',
    }),
    ...
    pnl_monthly_actuals_t: apiSource({
      adapter: 'xero_actuals',
      schema: S.pnl_monthly_actuals,
      description:
        'Per-month P&L actuals from Xero (Revenue / Cost of Sales / Gross Profit / ' +
        'Total Opex / EBITDA). Populated via "Sync from Xero" — stored in ' +
        '`ModelInput.actuals.pnlMonthly` between sessions. Any null column falls back ' +
        'to the modeled value in budget_monthly_with_actuals.',
    }),
    cost_dept_monthly_actuals_t: apiSource({
      adapter: 'xero_actuals',
      schema: S.cost_dept_monthly_actuals,
      description:
        'Per-(cost_dept, month_idx) opex actuals from Xero (staff / non_staff / total). ' +
        'Populated via "Sync from Xero" — stored in `ModelInput.actuals.costDeptMonthly`. ' +
        'Lets Xero replace modeled per-dept costs for completed months.',
    }),
    bs_monthly_actuals_t: apiSource({
      adapter: 'xero_actuals',
      schema: S.bs_monthly_actuals,
      description:
        'Per-month balance sheet actuals from Xero. Yearly view picks end-of-year ' +
        '(December) months. Populated via "Sync from Xero" — stored in ' +
        '`ModelInput.actuals.bsMonthly`.',
    }),
    cost_line_actuals_t: apiSource({
      adapter: 'xero_cost_line',
      schema: S.cost_line_actuals,
      description:
        'Per-(dept, line_name, month_idx) cost actuals at individual Xero account ' +
        'granularity (e.g. "G&A|Travel", "Engineering|Software"). Populated via ' +
  ...
  steps: [
    ...
    step({
      id: 'per_employee_monthly_intermediate',
    ...
    step({
      id: 'per_employee_monthly_intermediate_capped',
    ...
    step({
      id: 'per_employee_monthly_cost_raw',
    ...
    // ─── Cost line items × Xero actuals overlay ────────────────────────────
    step({
      id: 'cost_line_items_actuals_joined_t',
      name: 'Join cost line items with Xero actuals',
      description:
        'Left-join modeled cost lines with Xero account-level actuals on ' +
        '(dept, line_name, month_idx). Rows without a Xero actual get null actual_value.',
    ...
    step({
      id: 'cost_line_items_with_actuals_t',
      name: 'Apply Xero actuals to cost line items',
      description:
        'Use Xero actual_value when present; otherwise keep the modeled value. ' +
        'This is the effective cost-line table consumed by downstream aggregations.',
      input: 'cost_line_items_actuals_joined_t',
      op: ops.map({ value: coalesce(col('actual_value'), col('value')) }),
    }),
    ...
    step({
      ...
      description: 'Sum cost lines (Xero actuals applied where available; S&M Commission excluded).',
      input: 'cost_line_items_with_actuals_t',
    ...
    step({
      id: 'tangible_assets_monthly_full',
    ...
    step({
      id: 'retained_earnings_monthly_full',
  ...
  // groups and outputs are defined in src/view-layout/engine.yaml.
  // App.tsx merges them onto the model at runtime so the Engine explorer
  // sidebar and ★ headline outputs reflect the YAML source of truth.
  outputs: [],
  groups: [],
});
